...
int patchFlash0Archive()
{
	...
	fd = k_funcs.KernelIOOpen(path, PSP_O_RDONLY, 0777);

	if(fd < 0)
		return fd;

	k_funcs.KernelIORead(fd, &procfw_filecount, sizeof(procfw_filecount));
	...
	// skip file counter
	k_funcs.KernelIORead(fd, &fileSize, sizeof(fileSize));

	for(i=0; i<procfw_filecount; ++i)
	{
		ret = k_funcs.KernelIORead(fd, &fileSize, sizeof(fileSize));

		if(ret != sizeof(fileSize))
			break;

		ret = k_funcs.KernelIORead(fd, &lenFilename, sizeof(lenFilename));

		if(ret != sizeof(lenFilename))
			break;

		ret = k_funcs.KernelIORead(fd, namebuffer, lenFilename);
		...
		ret = k_funcs.KernelIORead(fd, contentbuffer, fileSize);
