...
int PatchSysMem(void *a0, void *sysmem_config)
{
	...
	u32 text_addr = ((u32)module_bootstart) - 0x10A14;

	/* Patch to add new partition */
	SetMemoryPartitionTable = (void *)text_addr + 0x1192C;
	_sw(MAKE_CALL(SetMemoryPartitionTablePatched), text_addr + 0x10CC4);

	/* Change attribute to 0xF (user accessable) */
	_sh(0xF, text_addr + 0x1134C);
